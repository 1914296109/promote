...
function Observe(obj) {
  ...
  // 通过 Object.keys(obj) 获取当前的 obj 上的每个属性
  Object.keys(obj).forEach(key => {
    ...
    Observe(value)
     ...
     Object.defineProperty(obj, key, {
       ...
       configurable: true,
       get () {
         // 只要执行了下面这一行，那么刚才new 的Watcher 实例，
...
function Compile (el,vm) {
  ...
  function replace(node) {
    ...
    // 证明当前 node 节点是一个文本子节点，需要进行正则的替换
    if(node.nodeType === 3) {
...
class Watcher {
  ...
  // 但是，只知道如何更新自己还不行，必须拿到最新的数据
  // 因此，还需要在new Watcher期间，把 vm 也传递进来，（因为 vm 中保存着最新的数据）
  // 除此之外，还需要知道，在 vm 中，哪个才是所需的属性
  ...
  constructor(vm,key,cb) {
    ...
    Dep.target = this
    key.split('.').reduce((newVal,k) => newVal[k],vm)
    Dep.target = null
  }

  // watcher 的实例，需要update函数，从而让发布者能够通知我们进行更新
  update () {
